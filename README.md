...
| --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| Policy    | `FOUNDRY_POLICY_MODE`, `FOUNDRY_ENFORCE_CANONICAL_HOST`, `FOUNDRY_INDEXABLE`                                                                                                                                                                            | Mode is inferred from `NODE_ENV`; no host normalisation              |
...
| `FOUNDRY_INDEXABLE`              | **leave unset** until DNS cutover | Indexing is opt-in (D-035). Unset means `noindex, nofollow` everywhere: `robots.txt` disallows all, every route emits noindex metadata, and the proxy adds an `X-Robots-Tag` header. Set it to `1` **only** on the real production deployment, at DNS cutover, alongside `FOUNDRY_ENFORCE_CANONICAL_HOST`. Set it at build time as well as runtime so the prerendered page metadata agrees with `robots.txt`. |
